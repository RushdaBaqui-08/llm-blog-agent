...
def try_stream(graph_app, inputs: Dict[str, Any]) -> Iterator[Tuple[str, Any]]:
    """
    Stream graph progress using stream_mode="updates".
    Accumulates final state and yields it as 'final' to avoid redundant execution.
    """
                ...
                if any(k in value for k in state_keys):
                    for sk, sv in value.items():
                        if sk == "sections" and isinstance(sv, list):
                            ...
                            if "evidence" not in current_state or not isinstance(current_state["evidence"], list):
                                current_state["evidence"] = []
                            current_state["evidence"].extend(sv)
                        else:
                            current_state[sk] = sv
                else:
